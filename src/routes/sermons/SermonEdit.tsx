...
import { sermonsApi, personnesApi, seriesApi, typesCulteApi } from "@/api";
import type {
  CitationBranham,
  PassageBiblique,
  PlanItem,
...
function fromSermon(sermon: Sermon): FormState {
  ...
  return {
    ...
    predicateur_id:
      typeof sermon.predicateur === "string"
        ? sermon.predicateur
        : sermon.predicateur?.id ?? "",
...
export function SermonEditPage() {
  ...
  const personnesQuery = useQuery({
    queryKey: ["personnes-actifs"],
    queryFn: () => personnesApi.list({ actif: true, page_size: 100 }),
  });
    ...
    <>
      ...
      <PageHead
        ...
        actions={
          <div className={common.actions}>
            ...
            {!isNew && (sermon?.statut === "en_revue" || sermon?.statut === "rejete") ? (
              <Button
                variant="success"
      ...
      <PageBody>
        <div className={common.editor}>
          <section className={common.section}>
            ...
            <div className={common.formGrid}>
              ...
              <Select
                ...
                help={
                  typesCulteQuery.isLoading
                    ? "Chargement…"
                    : "Gérez la liste depuis /admin/sermons/typeculte/ (admin Django)."
                }
              >
              ...
              <Select
                ...
                onChange={(event) => updateField("predicateur_id", event.target.value)}
              >
                <option value="">— Choisir —</option>
                {personnesQuery.data?.results.map((p) => (
          ...
          <section className={common.section}>
            <span className={common.sectionTitle}>§3 Passages bibliques (FR)</span>
          ...
          <section className={common.section}>
            <span className={common.sectionTitle}>§4 Citations Branham (FR)</span>
          ...
          <section className={common.section}>
            <span className={common.sectionTitle}>§5 Plan du message (FR)</span>
          ...
          {sermon ? (
            <section className={common.section}>
              <span className={common.sectionTitle}>§6 Audit</span>
